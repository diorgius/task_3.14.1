...
```
Наиболее интересной опцией является **format**, которая позволяет указать формат для вывода информации. Особенно это может быть полезным когда вы хотите сгенерировать вывод для автоматического анализа — так как вы указываете формат явно, он не будет изменён даже после обновления Git:
```
...
*Наиболее распространённые опции для команды git log:*![](/pict/gitlog.png "Наиболее распространённые опции для команды git log")
...
Также вы можете фильтровать список коммитов по заданным параметрам. Опция **--author** даёт возможность фильтровать по автору коммита, а опция --grep искать по ключевым словам в сообщении коммита.
...
```
*Опции для ограничения вывода команды git log*![](/pict/gitlogrestrict.png "Опции для ограничения вывода команды git log")
